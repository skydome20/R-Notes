...
1. 安裝**Python版本**的Tensorflow/Keras(運行深度學習模型的框架)

    ```
    1. 安裝Anaconda
    2. 在Anaconda下建立r-tensorflow的虛擬環境
    3. 在r-tensorflow環境內，用pip安裝tensorflow
    4. 在r-tensorflow環境內，安裝keras
    5. 測試python的tensorflow是否能夠使用
    ```


2. 安裝**R版本**的Tensorflow/Keras的(建立R跟Python之間的介面)

    ```
    1. 用R安裝Github上的rstudio/keras或rstudio/tensorflow
    2. 使用require(tensorflow)/require(keras)匯入套件
    3. 測試能否在R內使用tensorflow/keras
    ```
...
### (1) 安裝rstudio/keras或rstudio/tensorflow
